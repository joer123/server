...
char* rx_server_ajax(struct mg_connection* mc, char* ip_forwarded) {
    ...
    if (down && st->type != AJAX_VERSION && st->type != AJAX_STATUS && st->type != AJAX_USERS && st->type != AJAX_DISCOVERY)
        return NULL;
...
#define TMEAS(x) x
        ...
        if (data_len >= DX_UPLOAD_MAX_SIZE) {
            rc = 2;
            goto fail;
        }
...
#define N_CSV_FIELDS_SIG_BW 13
                    ...
                    if (n != N_CSV_FIELDS && n != N_CSV_FIELDS_SIG_BW) {
                        rc = 10;
                        goto fail;
                    }
                    ...
                    if (line != 0 || strncasecmp(qs[0].str, "freq", 4) != 0) {
                        ...
                        float freq;
                        if (_dx_parse_csv_field(CSV_FLOAT, qs[0].str, &freq)) {
                            rc = 11;
                            goto fail;
                        }
                        // printf("freq=%.2f\n", rem, freq);
                        ...
                        if (_dx_parse_csv_field(CSV_STRING, qs[1].str, &mode, CSV_EMPTY_NOK)) {
                            rc = 12;
                            goto fail;
                        }
                        if (_dx_parse_csv_field(CSV_DECODE, qs[2].str, &ident, CSV_EMPTY_NOK)) {
                            rc = 13;
                            goto fail;
                        }
                        if (_dx_parse_csv_field(CSV_DECODE, qs[3].str, &notes, CSV_EMPTY_OK)) {
                            rc = 14;
                            goto fail;
                        }
                        if (_dx_parse_csv_field(CSV_DECODE, qs[4].str, &ext, CSV_EMPTY_OK, &ext_empty)) {
                            rc = 15;
                            goto fail;
                        }

                        char* type;
                        if (_dx_parse_csv_field(CSV_STRING, qs[5].str, &type, CSV_EMPTY_OK, &empty)) {
                            rc = 16;
                            goto fail;
                        }
                        else {
                        ...
                        float pb_lo, pb_hi;
                        if (_dx_parse_csv_field(CSV_FLOAT, qs[6].str, &pb_lo)) {
                            rc = 17;
                            goto fail;
                        }
                        if (_dx_parse_csv_field(CSV_FLOAT, qs[7].str, &pb_hi)) {
                            rc = 18;
                            goto fail;
                        }
                        if (pb_lo != 0 || pb_hi != 0)
                        ...
                        float offset;
                        if (_dx_parse_csv_field(CSV_FLOAT, qs[8].str, &offset)) {
                            rc = 19;
                            goto fail;
                        }
                        if (offset != 0)
                        ...
                        char* dow_s;
                        if (_dx_parse_csv_field(CSV_STRING, qs[9].str, &dow_s, CSV_EMPTY_OK, &empty)) {
                            rc = 20;
                            goto fail;
                        }
                        else if (!empty) {
                        ...
                        float begin, end;
                        if (_dx_parse_csv_field(CSV_FLOAT, qs[10].str, &begin)) {
                            rc = 22;
                            goto fail;
                        }
                        if (_dx_parse_csv_field(CSV_FLOAT, qs[11].str, &end)) {
                            rc = 23;
                            goto fail;
                        }
                        ...
                        float sig_bw = 0;
                        if (n == N_CSV_FIELDS_SIG_BW && _dx_parse_csv_field(CSV_FLOAT, qs[12].str, &sig_bw)) {
                            rc = 24;
                            goto fail;
                        }
                        if (sig_bw != 0)
...
#define NKWDS 8
            ...
            kiwi_asfree(loc);
            kiwi_ifree(r_loc, "AJAX_STATUS r_loc");
            if (i == n) {
                char* name2;
